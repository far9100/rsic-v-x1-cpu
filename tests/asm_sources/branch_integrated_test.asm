...
beq_pass_1:
    addi x29, x0, 0     # 成功標記
beq_fail_1:
    sw x29, 0(x28)      # 儲存結果 (x29 應為 0 表示成功)
    
    # 測試 1.2: BEQ 失敗案例 - x2 != x4 不應分支
    addi x29, x0, 0     # 預設成功標記
...
beq_fail_2:
    addi x29, x0, 1     # 失敗標記
beq_pass_2:
    sw x29, 4(x28)      # 儲存結果 (x29 應為 0 表示成功)
...
bne_pass_1:
    addi x29, x0, 0     # 成功標記
bne_fail_1:
    sw x29, 8(x28)      # 儲存結果 (x29 應為 0 表示成功)
...
bne_fail_2:
    addi x29, x0, 1     # 失敗標記
bne_pass_2:
    sw x29, 12(x28)     # 儲存結果 (x29 應為 0 表示成功)
...
blt_pass_1:
    addi x29, x0, 0     # 成功標記
blt_fail_1:
    sw x29, 16(x28)     # 儲存結果 (x29 應為 0 表示成功)
...
blt_fail_2:
    addi x29, x0, 1     # 失敗標記
blt_pass_2:
    sw x29, 20(x28)     # 儲存結果 (x29 應為 0 表示成功)
...
bge_pass_1:
    addi x29, x0, 0     # 成功標記
bge_fail_1:
    sw x29, 24(x28)     # 儲存結果 (x29 應為 0 表示成功)
...
bge_pass_2:
    addi x29, x0, 0     # 成功標記
bge_fail_2:
    sw x29, 28(x28)     # 儲存結果 (x29 應為 0 表示成功)
...
bltu_pass_1:
    addi x29, x0, 0     # 成功標記
bltu_fail_1:
    sw x29, 32(x28)     # 儲存結果 (x29 應為 0 表示成功)
...
bltu_fail_2:
    addi x29, x0, 1     # 失敗標記
bltu_pass_2:
    sw x29, 36(x28)     # 儲存結果 (x29 應為 0 表示成功)
...
bgeu_pass_1:
    addi x29, x0, 0     # 成功標記
bgeu_fail_1:
    sw x29, 40(x28)     # 儲存結果 (x29 應為 0 表示成功)
...
bgeu_pass_2:
    addi x29, x0, 0     # 成功標記
bgeu_fail_2:
    sw x29, 44(x28)     # 儲存結果 (x29 應為 0 表示成功)
...
# 預期結果：
# 記憶體位址 0x100 應包含值 0 (BEQ 成功測試)
# 記憶體位址 0x104 應包含值 0 (BEQ 失敗測試)
# 記憶體位址 0x108 應包含值 0 (BNE 成功測試)
# 記憶體位址 0x10C 應包含值 0 (BNE 失敗測試)
# 記憶體位址 0x110 應包含值 0 (BLT 成功測試)
# 記憶體位址 0x114 應包含值 0 (BLT 失敗測試)
# 記憶體位址 0x118 應包含值 0 (BGE 成功測試)
# 記憶體位址 0x11C 應包含值 0 (BGE 成功測試 - 相等情況)
# 記憶體位址 0x120 應包含值 0 (BLTU 成功測試)
# 記憶體位址 0x124 應包含值 0 (BLTU 特殊測試 - 負數比較)
# 記憶體位址 0x128 應包含值 0 (BGEU 成功測試)
# 記憶體位址 0x12C 應包含值 0 (BGEU 特殊測試 - 負數比較)
# 記憶體位址 0x130 應包含值 3 (後向分支測試 - 迴圈計數) 